...
public class ApiTests : IClassFixture<CanteenApiFactory>
{
    ...
    {
        var response = await _client.PostAsJsonAsync("/api/meals", new CreateMealDto("Chicken Schnitzel", 120m));

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        var meal = await response.Content.ReadFromJsonAsync<MealDto>();
        Assert.NotNull(meal);
        Assert.Equal("Chicken Schnitzel", meal.Description);
    ...
    public async Task PutOrderStatus_InvalidTransition_ReturnsBadRequest()
    {
        ...
        // Cannot go directly from Preparing to Completed
